...
export default class PendulumVisualization extends Canvas {
  ...
  changeModel(newSimulation: module.PendulumSimulation) {
    this.simulation = newSimulation;
    this.initialize();
  }

  updateParameters(
    params: { damping: number; driveAmplitude: number; driveFrequency: number },
    speed: number,
    running: boolean
  ) {
    this.setSpeed(speed);
    this.setParameters(params.damping, params.driveAmplitude, params.driveFrequency);
  ...
  drawPendulum() {
    const { theta, drive } = this.simulation;
  ...
  drawPhaseHistory() {
    ...
    this.ctx.beginPath();
    let [lastT, lastO] = [theta, omega];
    for (let i = 1; i < this.simulation.historySize; i++) {
      const [thisT, thisO] = [this.simulation.theta_idx(i), this.simulation.omega_idx(i)];
  ...
  drawPoincareSection() {
    ...
    for (let i = 0; i < this.simulation.poincareSize; i++) {
      const [thisT, thisO] = [this.simulation.poincare_theta(i), this.simulation.poincare_omega(i)];
      const [x, y] = [thisT * scale + center.x, -thisO * scale + center.y];
    ...
    if (this.simulation.poincareSize) {
      const [theta, omega] = [this.simulation.poincare_theta(0), this.simulation.poincare_omega(0)];
      const [x, y] = [theta * scale + center.x, -omega * scale + center.y];
      if (!Canvas.outOfBounds(x, y, this.lowerRight)) {
        this.ctx.beginPath();
  ...
  drawPhasePoint(x: number, y: number, { current = false } = {}) {
